...
class tc_filelist extends s {
  ...
  render() {
    ...
    console.log(prev);
    return $`<strong>Path:${decodeURIComponent(this.url)}</strong><br><a class="dir" href="#${prev}">../</a><br>${this.files.map(
      (file) =>
        $`${o(file.type, [
          [
            "dir",
            () =>
              $`<a class="dir" href="#${this.url}/${file.name}/">${file.name}/</a>`,
          ],
          [
            "file",
            () =>
              $`<a class="file" href="/dav/${this.url}/${file.name}" download="${file.name}">${file.name}</a>`,
          ],
        ])}<br>`
    )}`;
  }
...
export { tc_filelist, tc_fileupload, tinycloud };
