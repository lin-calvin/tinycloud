...
class tc_filelist extends s {
  ...
  render() {
    ...
    console.log(prev);
    return $`${this.menu} <strong>Path:${decodeURIComponent(this.url)}</strong><br><div><a class="dir" href="#${prev}">../</a><br>${this.files.map(
        (file) =>
          $`${o(file.type, [
            [
              "dir",
              () =>
                $`<a tc-filename="${file.name}" class="dir" href="#${this.url}/${file.name}/">${file.name}/</a>`,
            ],
            [
              "file",
              () =>
                $`<a class="file" tc-filename="${file.name}" href="/dav/${this.url}/${file.name}" download="${file.name}">${file.name}</a>`,
            ],
          ])}<br>`
      )}</div>`;
  }
